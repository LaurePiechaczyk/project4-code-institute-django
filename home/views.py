...
from .models import Noun, GenderToDisplay, NounCategory
...
def index_with_gender_and_category(chosen_gender, chosen_category):
  nouns = Noun.objects.filter(categories__noun_category__contains = chosen_category).filter(gender__noun_gender__contains = chosen_gender).order_by('?')[:number_of_displayed_word]
...
def index_with_category(chosen_category):
  nouns = Noun.objects.filter(categories__noun_category__contains = chosen_category).order_by('?')[:number_of_displayed_word]
  categories = NounCategory.objects.all()
  context = {
    'nouns' : nouns,
    'categories' : categories,
...
def index_with_noun_list(chosen_gender):
  nouns = Noun.objects.filter(gender__noun_gender__contains = chosen_gender).order_by('?')[:number_of_displayed_word]
  categories = NounCategory.objects.all()
  context = {
    'nouns' : nouns,
    'categories' : categories,
